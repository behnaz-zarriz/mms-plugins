...
class AdminToolsController extends ActionController {
    ...
    public function FieldValsForElement() {
        ...
        while($qr_vals->nextRow()) {
            ...
            // Normalize display value to avoid counting formatting differences
            $vs_value = trim($vs_value);
            if (!isset($va_value_records[$vs_value][$va_row['row_id']])) {
                $va_value_counts[$vs_value]++;
                $va_value_records[$vs_value][$va_row['row_id']] = $va_row['value_id'];
            }
        }
    ...
    public function CreateSetForValue() {
        ...
        if($qr_val->nextRow()) {
            ...
            // Date ranges may have different raw database values while producing the
            // same display value (e.g. "1895-1905" -> "1895 bis 1905").
            // Therefore compare the rendered display value instead of the stored values.
            if ((int)$t_element->get('datatype') === __CA_ATTRIBUTE_VALUE_DATERANGE__) {
                $o_selected_val = new DateRangeAttributeValue($va_row);
                $vs_selected_display_value = trim($o_selected_val->getDisplayValue());

                $qr_all_vals = $o_db->query('
		        SELECT ca_attributes.row_id, cav.*
		        FROM ca_attributes
		        INNER JOIN ca_attribute_values cav ON ca_attributes.attribute_id = cav.attribute_id
		        WHERE ca_attributes.table_num = ?
		          AND cav.element_id = ?
		    ', $pn_table_num, $pn_element_id);

                $pa_ids = [];
                while($qr_all_vals->nextRow()) {
                    // Match all records with the same rendered date range
                    $va_candidate = $qr_all_vals->getRow();
                    $o_candidate_val = new DateRangeAttributeValue($va_candidate);
                    $vs_candidate_display_value = trim($o_candidate_val->getDisplayValue());

                    if ($vs_candidate_display_value === $vs_selected_display_value) {
                        $pa_ids[] = $va_candidate['row_id'];
                    }
                }


                /// Remove duplicate object ids before creating the set
                ...
                $pa_ids = array_values(array_unique($qr_all_vals->getAllFieldValues('row_id')));
            }
            ...
            // Add all matching objects to the newly created set
